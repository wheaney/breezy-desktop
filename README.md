...
**As a flake input:**
```nix
inputs.breezy-desktop.url = "github:wheaney/breezy-desktop";
```
then either install it imperatively with `nix profile`/`nix-env`:
```bash
nix profile install github:wheaney/breezy-desktop#breezy-desktop
```
...
```bash
nix-env -f https://github.com/wheaney/breezy-desktop/archive/main.tar.gz -iA packages.x86_64-linux.breezy-desktop
```
or as an overlay:
```nix
self: super:
  (import (fetchTarball "https://github.com/wheaney/breezy-desktop/archive/main.tar.gz") { }).overlays.default self super
```
