...
gboolean update_crop (gpointer data) {
    ...
    if (captured_starting_euler) {
        GstElement *videobox = GST_ELEMENT(data);
...
void stream_video() {
    GMainLoop *loop;
    GstElement *pipeline, *source, *sink, *videobox, *videoscale, *caps_filter;
    GstCaps *video_caps;
    GstBus *bus;
    ...
    video_caps = gst_caps_new_simple(
        ...
        "width", G_TYPE_INT, (gint)display_width,
        "height", G_TYPE_INT, (gint)display_height,
        NULL);
    sink = gst_element_factory_make ("ximagesink", "sink");

    if (!pipeline || !source || !videobox || !sink || !videoscale || !caps_filter || !video_caps) {
        g_printerr ("One element could not be created. Exiting. %d %d %d %d %d %d %d\n", !pipeline, !source, !videobox, !sink, !videoscale, !caps_filter, !video_caps);
        exit(1);
    }
...
//    gst_object_unref(pad);
    ...
    gst_bin_add_many (GST_BIN (pipeline), source, videobox, videoscale, caps_filter, sink, NULL);

    if (!gst_element_link_many (source, videobox, videoscale, caps_filter, sink, NULL)) {
        g_printerr ("Elements could not be linked. Exiting.\n");
        exit(1);
    }

    gst_element_set_state (pipeline, GST_STATE_PLAYING);
    g_timeout_add(1000 / head_movement_fps, update_crop, videobox);
