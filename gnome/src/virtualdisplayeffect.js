...
// TODO - fix size (or maybe distance) of flat display
...
function createVertexMesh(fovDetails, monitorDetails, positionVectorNWU) {
    ...
    function v(s, t) {
        ...
        const xOffsetPixels = fovDetails.curvedDisplay && horizontalWrap ? Math.sin(xOffsetRadians) * radius : monitorDetails.width * xOffset;
        const x = -positionVectorNWU[1] + xOffsetPixels;

        const yOffset = 0.5 - t;
        const yOffsetRadians = yOffset * verticalRadians;
        const yOffsetPixels = fovDetails.curvedDisplay && verticalWrap ? Math.sin(yOffsetRadians) * radius : monitorDetails.height * yOffset;
        const y = positionVectorNWU[2] + yOffsetPixels;

        let z = -positionVectorNWU[0];
        if (horizontalWrap || verticalWrap) {
            const relevantLength = horizontalWrap ? xOffsetPixels : yOffsetPixels;
            z += radius - Math.sqrt(radius * radius - relevantLength * relevantLength);
        }

        Globals.logger.log_debug(`\t\t\tvertex: ${JSON.stringify({x, y, z, s, t})}`);
...
export const VirtualDisplayEffect = GObject.registerClass({
    ...
    vfunc_paint_target(node, paintContext) {
        if (this.imu_snapshots && !Globals.data_stream.show_banner) {
            if (!this._initialized) {
